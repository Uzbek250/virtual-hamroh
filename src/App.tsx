...
export default function App() {
  ...
  const live = useLiveConversation({ userId, voiceName, onAction: handleLiveAction });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (Array.isArray(data.messages) && data.messages.length) setMessages(data.messages.slice().reverse().map((m: any) => ({ id: m.id, role: m.role, text: m.text, emotion: m.emotion, timestamp: m.timestamp })));
        if (Array.isArray(data.reminders) && data.reminders.length) setReminders(data.reminders.slice().reverse().map((r: any) => ({ id: r.id, text: r.text, timeString: r.timeString ?? r.time_string, dateTime: r.dateTime ?? r.date_time, triggered: Boolean(r.triggered), createdAt: r.createdAt ?? r.created_at })));
        if (Array.isArray(data.moods) && data.moods.length) setMoodEntries(data.moods.slice().reverse().map((m: any) => ({ id: m.id, mood: m.mood, note: m.note, date: m.date, timestamp: m.timestamp })));
      } catch (error) {
  ...
  useEffect(() => {
    ...
    rec.lang = "uz-UZ"; rec.continuous = false; rec.interimResults = false;
    rec.onstart = () => { setIsRecording(true); setBotState("listening"); };
    rec.onresult = (event: any) => { const transcript = event.results[0][0].transcript; if (transcript) setCurrentMessage((prev) => (prev ? `${prev} ${transcript}` : transcript)); };
  ...
  useEffect(() => {
    const interval = window.setInterval(() => {
      ...
      setReminders((prev) => prev.map((rem) => {
        ...
        if (!rem.triggered && now >= remTime) { setActiveAlert(rem); playAlarmSound(); return { ...rem, triggered: true }; }
        return rem;
      }));
    }, 15_000);
  ...
  const playAlarmSound = () => {
    try {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        osc.frequency.setValueAtTime(freq, now + i * 0.4); gain.gain.setValueAtTime(0, now + i * 0.4);
        gain.gain.linearRampToValueAtTime(0.15, now + i * 0.4 + 0.05); gain.gain.exponentialRampToValueAtTime(0.001, now + i * 0.4 + 0.35);
  ...
  const toggleRecording = () => {
    if (!recognitionRef.current) { showToast("Kechirasiz, ushbu brauzerda ovoz kiritish qo'llab-quvvatlanmaydi. Iltimos Chrome brauzeridan foydalaning."); return; }
    if (isRecording) recognitionRef.current.stop(); else { currentAudioRef.current?.pause(); recognitionRef.current.start(); }
  };

  const stopAudio = () => { currentAudioRef.current?.pause(); if ("speechSynthesis" in window) window.speechSynthesis.cancel(); setBotState("idle"); };
  ...
  const playSpeechFallback = (text: string) => {
    ...
    try {
      ...
      utterance.onstart = () => setBotState("speaking"); utterance.onend = () => setBotState("idle"); utterance.onerror = () => setBotState("idle");
  ...
  const handleAddManualReminder = (e: React.FormEvent) => {
    e.preventDefault(); if (!manualReminderText || !manualReminderTime) return;
    ...
    const newRem: Reminder = { id: crypto.randomUUID(), text: manualReminderText, timeString: parsedTime.toLocaleString("uz-UZ", { hour: "2-digit", minute: "2-digit", day: "2-digit", month: "2-digit" }), dateTime: parsedTime.toISOString(), triggered: false, createdAt: new Date().toISOString() };
    setReminders((prev) => [newRem, ...prev]); setManualReminderText(""); setManualReminderTime(""); setShowAddReminderModal(false);
  ...
  const handleAddManualMood = (e: React.FormEvent) => {
    e.preventDefault(); if (!manualMoodNote) return;
    ...
    setMoodEntries((prev) => [newEntry, ...prev]); setManualMoodNote(""); setShowAddMoodModal(false);
  ...
  const handleDeleteReminder = (id: string) => { setReminders((prev) => prev.filter((r) => r.id !== id)); fetch(`/api/reminders/${encodeURIComponent(id)}?userId=${encodeURIComponent(userId)}`, { method: "DELETE" }).catch(console.warn); };
  const handleDeleteMood = (id: string) => { setMoodEntries((prev) => prev.filter((m) => m.id !== id)); fetch(`/api/moods/${encodeURIComponent(id)}?userId=${encodeURIComponent(userId)}`, { method: "DELETE" }).catch(console.warn); };

  const handleSendMessage = async (e?: React.FormEvent) => {
    e?.preventDefault(); if (!currentMessage.trim() || isLoading) return;
    const userText = currentMessage.trim(); setCurrentMessage(""); stopAudio();
    ...
    try {
      ...
      const data = await response.json();
      const robotReply = data.reply; const robotEmotion = data.emotion as EmotionType; const robotAction = data.action;
      ...
      if (robotAction && robotAction.type !== "none") {
        ...
        if (robotAction.type === "eslatma" && payload) {
          const rawTime = payload.time || "ertaga 09:00"; const parsedTargetTime = parseUzbekTime(rawTime);
      ...
      if (data.audio && ttsEnabled) {
        try {
          const binary = atob(data.audio); const bytes = new Uint8Array(binary.length); for (let i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i);
          const url = URL.createObjectURL(new Blob([bytes], { type: data.audioMimeType || "audio/wav" })); const audio = new Audio(); currentAudioRef.current = audio; setBotState("speaking");
      ...
      } else if (ttsEnabled && robotReply) playSpeechFallback(robotReply); else setBotState("idle");
    } catch (err: any) {
